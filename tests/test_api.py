...
def test_crearworkflow():
    ...
    assert response.json()["results file"] == "data/workflow_results.txt"
...
def test_cargarworkflowdesdejson():
    response = requests.post("http://localhost:8000/cargarworkflowdesdejson", json={"archivo_json": "data/workflow.json"})
    assert response.status_code == 200
